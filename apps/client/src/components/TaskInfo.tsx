...
function TaskInfo({ taskId, title, description, type, priority, dueDate, assigneeId, onTaskDeleted, onClose, onDescriptionUpdated, darkMode = false }: TaskInfoProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`border-b pb-2 mb-3 ${darkMode ? 'border-[#2C2C2C]' : 'border-gray-200'}`}>
        <motion.h2 
      ...
      >
        ...
        >
          <div className={`h-5 w-5 flex items-center justify-center mr-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            <svg className={`w-3 h-3 ${getPriorityColor(priority)}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          ...
          <span className={`text-xs font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>
            {assigneeId ? assigneeId.length > 10 ? assigneeId.substring(0, 10) + '...' : assigneeId : 'Unassigned'}
          </span>
        </motion.div>
        ...
        >
          <div className={`h-5 w-5 flex items-center justify-center mr-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            <svg className={`w-3 h-3 ${timeStatus.isOverdue ? 'text-red-500' : 'text-emerald-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <span className={`text-xs font-medium ${timeStatus.isOverdue ? (darkMode ? 'text-red-400' : 'text-red-500') : (darkMode ? 'text-white' : 'text-gray-800')}`}>
            {timeStatus.text}
          </span>
        </motion.div>
      </motion.div>
