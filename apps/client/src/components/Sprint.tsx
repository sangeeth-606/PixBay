...
const Sprint: React.FC<SprintProps> = ({ sprintId, darkMode }) => {
  ...
  const handleCreateTask = async () => {
    try {
      ...
      setSprint((prev) =>
        prev ? { ...prev, tasks: [...prev.tasks, createdTask] } : prev,
      );
  ...
  if (loading)
    return (
      <div className={`${darkMode ? "text-white" : "text-gray-800"} p-6`}>
        Loading...
      </div>
    );
  ...
  const totalStoryPoints =
    sprint.tasks?.reduce((sum, task) => sum + (task.storyPoints || 0), 0) || 0;
  ...
  const taskCounts = {
    ...
    inProgress:
      sprint.tasks?.filter((task) => task.status === "IN_PROGRESS").length || 0,
  ...
  const shadow = darkMode
    ? "shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_0_1px_0_rgba(255,255,255,0.05)]"
    : "shadow-sm";
  ...
  return (
    <motion.div
      className={`${baseBg} ${textColor} p-6 rounded-md w-full`}
      initial="initial"
      animate="animate"
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">
            {sprint.name} - {sprint.project.key}
          </h1>
          <h2 className={`text-sm ${secondaryTextColor}`}>
            Owned by: {sprint.owner.name}
          </h2>
      ...
      <motion.div
        className={`${cardBg} p-6 rounded-md mb-6 ${shadow}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <motion.div
          className={`${cardBg} p-6 rounded-md ${shadow}`}
          variants={itemVariants}
        >
          ...
          <div
            className={`w-full ${darkMode ? "bg-[#171717]" : "bg-gray-200"} rounded-full h-2 mb-2`}
          >
            <div
        ...
        >
          ...
          <div className="flex justify-between">
            <div className="text-center">
              ...
              <div className={`text-sm ${secondaryTextColor}`}>To Do</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{taskCounts.inProgress}</div>
              <div className={`text-sm ${secondaryTextColor}`}>In Progress</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{taskCounts.done}</div>
              <div className={`text-sm ${secondaryTextColor}`}>Done</div>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="mb-6">
        ...
        {showCreateTask && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div
              className={`${cardBg} p-6 rounded-md w-full max-w-md ${shadow}`}
            >
          ...
          <tbody className={`divide-y ${borderColor}`}>
            {sprint.tasks?.map((task) => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  {task.assignee ? (
                    <div className="flex items-center">
                      <span
                        className={`w-6 h-6 ${darkMode ? "bg-[#3C3C3C]" : "bg-gray-200"} rounded-full flex items-center justify-center text-xs mr-2`}
                      >
                ...
                <td className="py-3 px-4">
                  <button
                    className={`${darkMode ? "text-blue-400 hover:underline" : "text-blue-600 hover:underline"} mr-2`}
                    onClick={() => alert(`Edit task ${task.id}`)}
                  >
                    Edit
                  </button>
                  <button
                    className={`${darkMode ? "text-red-400 hover:underline" : "text-red-600 hover:underline"}`}
                    onClick={() => alert(`Move task ${task.id} to backlog`)}
                  >
                    Move
                  </button>
                </td>
